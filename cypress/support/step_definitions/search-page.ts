...
When("I filter cars list by {string} model filter using valid dates", (model) => {
...
Then("I should see the table with only particular {string} cars", (model) => {
  cy.get("tbody > tr").each(($el) => {
    expect($el).to.contain(model)
 })
})

Then("I should see the alert saying {string}", (alert) => {
