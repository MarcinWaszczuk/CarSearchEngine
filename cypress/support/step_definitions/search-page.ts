...
When('I filter cars list by {string} model filter using valid dates', (model: string) => {
...
Then('I should see the table with only particular {string} cars', (model: string) => {
  cy.get('tbody > tr').each($el => {
    expect($el).to.contain(model)
  })
})

Then('I should see the alert saying {string}', (alert: string) => {
